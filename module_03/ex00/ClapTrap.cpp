...
ClapTrap::ClapTrap() {
	std::cout << "Default constructor called to address: " << this << std::endl;
	_name = "NoName";
	_hit_points = 0;
	_energy_points = 0;
	_attack_damage = 0;
...
ClapTrap::ClapTrap(const ClapTrap &copy) {
	std::cout << "Copy's constructor called to address: " << this << std::endl;
	_name 			= copy._name;
...
ClapTrap &ClapTrap::operator=(const ClapTrap &overload) {
	std::cout << "An assignation operator overload called" << std::endl;
	if (this == &overload)
		return *this;
...
void ClapTrap::beRepaired(unsigned int amount) {
	...
	<< " points." << " It's got " << _energy_points << " energy points!" <<
	std::endl;
}
